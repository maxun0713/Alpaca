...
int CNode::Recv(char *peerNodeID, size_t &peerNodeIDBufLen,
                void *dataBuf, size_t &dataLen, bool noWait)
{
	zmq_msg_t zmsg;
	...
	if (ret < 0)
	{
		if (errno != EAGAIN)
		{
			SET_ERR_MSG(_lastErrMsg, "recv delimiter failed:" << strerror(errno))
		}
		zmq_msg_close(&zmsg);
		return ret;
	}
                 ...
                 const char *data, size_t dataLen)
{
    ...
    if (ret < 0)
    {
        ...
        {
            SET_ERR_MSG(_lastErrMsg,
                        "send data msg failed!" << strerror(errno))
        }
        return  ret;
    }
...
{
	_dataBufLen = params.dataBufLen > DEFAULT_NODE_DATA_BUF_LEN_MIN
	...
	_peerNodeIDBufLen = params.peerIDBufLen > DEFAULT_NODE_ID_BUF_LEN_MIN
		? params.peerIDBufLen : DEFAULT_NODE_ID_BUF_LEN;
	_peerNodeIDBuf = static_cast<char*>(malloc(_peerNodeIDBufLen));
	if (_peerNodeIDBuf == NULL)
	{
		SET_ERR_MSG(_lastErrMsg, "malloc peer node id buf :" << _peerNodeIDBufLen)
		return -1;
	}
